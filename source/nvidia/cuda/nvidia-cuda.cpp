...
#define CUDA_LOAD_SYMBOL(NAME)                                                            \
	{                                                                                     \
		NAME = static_cast<decltype(NAME)>(_library->load_symbol(#NAME));                 \
		if (!NAME)                                                                        \
			throw std::runtime_error("Failed to load '" #NAME "' from '" CUDA_NAME "'."); \
	}
#define CUDA_LOAD_SYMBOL_V2(NAME)                                                         \
	{                                                                                     \
		NAME = static_cast<decltype(NAME)>(_library->load_symbol(#NAME "_v2"));           \
		if (!NAME)                                                                        \
			throw std::runtime_error("Failed to load '" #NAME "' from '" CUDA_NAME "'."); \
	}
#define CUDA_LOAD_SYMBOL_EX(NAME, OVERRIDE)                                               \
	{                                                                                     \
		NAME = static_cast<decltype(NAME)>(_library->load_symbol(#OVERRIDE));             \
		if (!NAME)                                                                        \
			throw std::runtime_error("Failed to load '" #NAME "' from '" CUDA_NAME "'."); \
	}
